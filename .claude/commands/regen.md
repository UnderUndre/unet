...
Thin wrapper. The real work lives in the `clai-helpers` CLI (installed locally via `npm link` in the monorepo workspace, OR globally via `npm install -g clai-helpers`, OR ad-hoc via `npx clai-helpers`):

```bash
helpers regen [--dry-run] [--target <id>] [--verbose]
# OR — without install:
npx clai-helpers regen [--dry-run] [--target <id>] [--verbose]
```

> **Note on the CLI's repo location**: the `helpers` binary is published from `packages/cli/` in the `clai-helpers` monorepo (sibling repo, NOT in this template repo). In this `.claude/`-only template repo, run via `npx` to fetch from npm. In the monorepo, `helpers` is on PATH after `npm install`.
...
3. **Dry-run first** (when the change set is expected to be non-trivial or when debugging):
   ```bash
   helpers regen --dry-run     # or: npx clai-helpers regen --dry-run
   ```
4. **Execute**:
   ```bash
   helpers regen               # or: npx clai-helpers regen
   ```
